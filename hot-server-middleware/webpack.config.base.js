...
const server = {
  ...
  devtool: 'source-map',
  context: cwd,
  entry: { server: [(path.join(cwd, 'src', 'server', 'main.js'))] },
  output: {
    path: path.resolve(path.join(cwd, 'dist')),
    publicPath: '/assets/dist/',
    filename: 'server.bundle.js',
    libraryTarget: 'commonjs2' // lets export things!
  },
  plugins: [
  ...
  optimization: {
    nodeEnv: false // prevent overriding NODE_ENV based on webpack mode
  },
  node: {
    __dirname: true,
    __filename: true
